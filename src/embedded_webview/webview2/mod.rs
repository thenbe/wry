...
use webview2_com::{
  ...
  ClearBrowsingDataCompletedHandler, ContentLoadingEventHandler, CoreWebView2EnvironmentOptions,
  CreateCoreWebView2ControllerCompletedHandler, CreateCoreWebView2EnvironmentCompletedHandler,
  DownloadStartingEventHandler, ExecuteScriptCompletedHandler,
  Microsoft::Web::WebView2::Win32::{
    CreateCoreWebView2EnvironmentWithOptions, ICoreWebView2, ICoreWebView2Controller,
    ICoreWebView2Controller2, ICoreWebView2Environment, ICoreWebView2Environment10,
    ICoreWebView2Environment9, ICoreWebView2EnvironmentOptions, ICoreWebView2Profile2,
    ICoreWebView2Settings2, ICoreWebView2Settings3, ICoreWebView2Settings5, ICoreWebView2Settings6,
    ICoreWebView2WebResourceResponse, ICoreWebView2WindowCloseRequestedEventHandler,
    ICoreWebView2_10, ICoreWebView2_13, ICoreWebView2_4, COREWEBVIEW2_COLOR,
    COREWEBVIEW2_DOWNLOAD_STATE, COREWEBVIEW2_DOWNLOAD_STATE_COMPLETED,
    COREWEBVIEW2_DOWNLOAD_STATE_IN_PROGRESS, COREWEBVIEW2_MOVE_FOCUS_REASON_PROGRAMMATIC,
    COREWEBVIEW2_PERMISSION_KIND_CLIPBOARD_READ, COREWEBVIEW2_PERMISSION_KIND_UNKNOWN_PERMISSION,
    COREWEBVIEW2_PERMISSION_STATE_ALLOW, COREWEBVIEW2_PREFERRED_COLOR_SCHEME_AUTO,
  ...
  PermissionRequestedEventHandler, StateChangedEventHandler, WebMessageReceivedEventHandler,
...
use windows::{
  ...
  s,
  Win32::{
    Foundation::{BOOL, E_FAIL, E_POINTER, FARPROC, HWND, LPARAM, LRESULT, RECT, WPARAM},
    ...
    UI::{
      ...
      WindowsAndMessaging::{
        ...
        HICON, HMENU, SWP_ASYNCWINDOWPOS, SWP_NOACTIVATE, SWP_NOSIZE, SWP_NOZORDER,
...
use super::{EmbeddedWebViewAttributes, Theme};
use crate::{
  webview::{PageLoadEvent, ProxyConfig, RequestAsyncResponder, WebContext, RGBA},
  Error,
};

pub struct InnerEmbeddedWebview {
  pub parent: HWND,
  pub window: HWND,
...
impl InnerEmbeddedWebview {
  pub(crate) fn new(
    ...
    let parent = match parent {
      raw_window_handle::RawWindowHandle::Win32(p) => HWND(p.hwnd as _),
      _ => unimplemented!(),
    };

    let window = unsafe { Self::create_child_window(parent, &attributes)? };

    let _ = unsafe { CoInitializeEx(None, COINIT_APARTMENTTHREADED) };

    let env = Self::create_environment(&web_context, pl_attrs.clone(), &attributes)?;
    let controller = Self::create_controller(window, &env, attributes.incognito)?;
    let webview = Self::init_webview(window, attributes, &env, &controller, pl_attrs)?;
  ...
  unsafe fn register_window_class(window_classname: &str) -> Vec<u16> {
    let class_name = encode_wide(window_classname);
  ...
  unsafe fn create_child_window(
    parent: HWND,
    attrs: &EmbeddedWebViewAttributes,
  ) -> crate::Result<HWND> {
    let class_name = Self::register_window_class("WRY_EMBEDDED_WEBVIEW");
    let hwnd = CreateWindowExW(
      WINDOW_EX_STYLE::default(),
      PCWSTR::from_raw(class_name.as_ptr()),
      PCWSTR::null(),
      WS_CHILD | WS_VISIBLE,
      attrs.x.unwrap_or(CW_USEDEFAULT),
      attrs.y.unwrap_or(CW_USEDEFAULT),
      attrs.width.map(|w| w as i32).unwrap_or(CW_USEDEFAULT),
      attrs.height.map(|h| h as i32).unwrap_or(CW_USEDEFAULT),
      parent,
      HMENU::default(),
  ...
  fn init_webview(
    hwnd: HWND,
    mut attributes: EmbeddedWebViewAttributes,
    env: &ICoreWebView2Environment,
...
macro_rules! get_function {
  ($lib:expr, $func:ident) => {
    crate::embedded_webview::webview2::get_function_impl(
      concat!($lib, '\0'),
